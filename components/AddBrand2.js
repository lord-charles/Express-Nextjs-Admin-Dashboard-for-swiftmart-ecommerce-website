...
export default function AddBrand({ handleNext }) {
  ...
  useEffect(() => {
    const getsuccessFromLocalStorage =
      typeof window !== 'undefined' && localStorage.getItem('successRate')
        ...
        : null
    setSuccessFromLocalStorage(getsuccessFromLocalStorage.state.successRateBrand)
  }, [])
  ...
  console.log(brandData, brandSuccess)
  ...
  const handleAddProductDetails = () => {
    ...
    setBtnActive(false)
    createBrand(brandData, token.state.userDetails.token, setSuccessRateBrand, setBtnActive, handleNextPage)
  }
